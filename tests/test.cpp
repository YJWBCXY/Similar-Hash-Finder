...
TEST(sha256, OneChunkMessageBlock) {
    std::vector<std::string>
        input = {"Lorem ipsum dolor sit amet, consectetur adipiscing ut.",
        ...
        test =
            {"59fff55d6ad9adffb0a5cf64c457ca85c97406884ec023611820c9d0846a4597",
             ...
             "5217b27bd5a87930dee9fb61591e05f19a39eb3b46ca729be5befa4cd120d52"
             "3"},
        result;
    ...
    std::vector<std::string>
        input =
            {"Lorem ipsum dolor sit amet, consectetur adipiscing elit. In "
             "ornare metus sapien, vitae semper mi posuere at massa nunc.",
             "Lorem ipsum dolor sit amet, consectetur adipiscing elit. "
             "Vestibulum placerat tortor eu sem rutrum cursus. Cras sodales "
             "magna enim, id feugiat augue consectetur vitae. Orci varius nam.",
             "}.v6(D?1=PD$pK/"
             "(&8QV6HQ9XtqpdANBJyN?mmy_:+_cm0JtU;UmN8mGW6r+?7iN=(]b%]a+{.T!U7,("
             "i@:{kMx(-72eGr2SweyT3%rw5Bvt,LySGz@=_i7wg4RRUN,Ehr#L+h/"
             "ad=)r8]SN?(V",
             "qE3=hiP,D}P!gvfAUv#WwL.yM5i?kuyTyZ#C0&B$U}[{jAr1]yhRBi[938]a;"
             "TagHW)];Zwt*[0&",
             "cVyn#dt;?6c1hxjZ*G#:ar_M,-wdRuL&[TR_AX@Y;FV%p,JtF8&tE@"
             "3bBQc7Tra1v&}k04Pz-0JByUtauHufQqHA,T$bB,(62k;k",
             "=0:0ixwq%[Mh9A.7(!!!Ef-U75[$jw20Y5vQwV{J2WF:=)%$9NBCN,b727z$#j%]"
             "aE81Y2?d04X2Hi3Ah.3b[EX$vuHAE*{AL14/+;,e68(QBB&RH",
             "M:#t)QyMBy(en*w.%{7Mtk,!&y5)rKM)tt{W{!gNbz)5Dd,5_D!xBZg+.k8",
             "}2HKXrSn.(%V[tYXt+:adwiM@4av2t()3#Gn,?V68%A?H[hj4NFG6KV5E9*Zg?"
             "tgW&we]6S&Y2u[3eJEhaHu{LnPg[6q6$umPw#d/T0!.rU"},
        test =
            {"3541ab899ffbc19fab75d9f3bba6cb389056d441bbcc426713339ce6cbd5c5ad",
             ...
             "bc9bc5ef051236a4725df6c65ca7ac5d885918b7afbe97b0a389b4719cdf388"
             "a"},
        result;
    int lenght = input.size();
    for (int i = 0; i < lenght; i++) {
    ...
    std::vector<std::string>
        ...
        test =
            {"0101100111111111111101010101110101101010110110011010110111111111"
             ...
             "111001011011111011111010010011001101000100100000110101010010001"
             "1"},
        result;
    int lenght = input.size();
    for (int i = 0; i < lenght; i++) {
...
TEST(sha256, Base91Output) {
    std::vector<std::string>
        input = {"Lorem ipsum dolor sit amet, consectetur adipiscing ut.",
                 ...
                 ".kAi:nu/]L3@U$HYM=8Kj",
                 "w*RSEn},,17J"},
        test = {"R}Ls/:^={sB1M2F*IdYiM]Nt/NayYEaLUv!4:wdD",
...
TEST(Tools, BitRotateRight) {
    std::vector<std::bitset<32>> input = {std::bitset<32>(1426881987),
                                          std::bitset<32>(3144134277),
                                          std::bitset<32>(3600352804),
                                          std::bitset<32>(607225278)};
    std::vector<std::vector<std::bitset<32>>> test{
        {std::bitset<32>(1788972984),
         std::bitset<32>(2109953292),
         std::bitset<32>(1893024543),
         std::bitset<32>(838274388)},
        {std::bitset<32>(3077371344),
         std::bitset<32>(2927999847),
         std::bitset<32>(2708396523),
         std::bitset<32>(2662995693)},
        {std::bitset<32>(2597527748),
         std::bitset<32>(103077529),
         std::bitset<32>(2301994561),
         std::bitset<32>(1679332186)},
        {std::bitset<32>(3297128631),
         std::bitset<32>(2243830833),
         std::bitset<32>(1871252577),
         std::bitset<32>(3323394192)}},
        result(4);
...
TEST(Tools, BitAdder) {
    std::vector<std::bitset<32>> input = {std::bitset<32>(2428436474),
                                          std::bitset<32>(3336571891),
                                          std::bitset<32>(1996064986),
                                          std::bitset<32>(3584528711)};
    std::vector<std::vector<std::bitset<32>>> test{
        {std::bitset<32>(561905652),
         std::bitset<32>(1470041069),
         std::bitset<32>(129534164),
         std::bitset<32>(1717997889)},
        {std::bitset<32>(1470041069),
         std::bitset<32>(2378176486),
         std::bitset<32>(1037669581),
         std::bitset<32>(2626133306)},
        {std::bitset<32>(129534164),
         std::bitset<32>(1037669581),
         std::bitset<32>(3992129972),
         std::bitset<32>(1285626401)},
        {std::bitset<32>(1717997889),
         std::bitset<32>(2626133306),
         std::bitset<32>(1285626401),
         std::bitset<32>(2874090126)},
    },
        result(4);
