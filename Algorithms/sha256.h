...
std::vector<std::string> sha256_str_to_bin(std::string& input) {
    std::vector<std::string> tmp, out;
    int i = 0, k = 1, left, size;
...
std::bitset<32> bit_rotate_right(const std::bitset<32>& value,
                                 const int& shift) {
    return (value >> shift) | (value << (32 - shift));
}

std::bitset<32> bit_adder(const std::bitset<32>& a, const std::bitset<32>& b) {
    std::bitset<32> result, carry = 0;
    int i = 0;
    for (; i < 31; i++) {
...
std::vector<std::bitset<32>> sha256(std::string message) {
    ...
    // hash value
    std::vector<std::bitset<32>> hash = {std::bitset<32>(1779033703),
    ...
    while (i < lenght) {
        // working variables
        std::bitset<32> a = hash[0], b = hash[1], c = hash[2], d = hash[3],
        ...
        while (w16 < 64) {
            sigma0 = bit_rotate_right(w[w1], 7) ^ bit_rotate_right(w[w1], 18) ^
                     (w[w1] >> 3);
            sigma1 = bit_rotate_right(w[w14], 17) ^
                     bit_rotate_right(w[w14], 19) ^ (w[w14] >> 10);
            w[w16] =
                bit_adder(bit_adder(bit_adder(w[w0], sigma0), w[w9]), sigma1);
            w0++;
            w1++;
            w9++;
            w14++;
        ...
        for (int j = 0; j < 64; j++) {
            sigma0 = bit_rotate_right(a, 2) ^ bit_rotate_right(a, 13) ^
                     bit_rotate_right(a, 22);
            sigma1 = bit_rotate_right(e, 6) ^ bit_rotate_right(e, 11) ^
                     bit_rotate_right(e, 25);
            choice = (e & f) ^ ((~e) & g);
            majority = (a & b) ^ (a & c) ^ (b & c);
            tmp1 = bit_adder(
                bit_adder(bit_adder(bit_adder(h, sigma1), choice), k[j]),
                w[j]);
...
std::string sha256_to_base91(std::string message) {
    std::string bin_hash = sha256_to_bin(message), out_hash = "";
